...
import {  doc, getDoc, getDocs, setDoc, collection, updateDoc, addDoc, deleteDoc } from "firebase/firestore";
...
export const productsApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getProduct: builder.query<Product,string>({
            async queryFn(id: string): Promise<{ data: Product; error?: undefined; } | { error: any; data?: undefined; }> {
                try {
                    ...
                    const product = await docSnapShot.data() as Product;
                    const category = await getDoc(doc(db, 'categories', product?.category));
                    let c = await category.data();
                    const postedBy = await getDoc(doc(db, 'users', product?.postedBy? product.postedBy: ""));
                    let p = {
                        id: postedBy.id,
                        ...postedBy.data()
                    }
                    return { data: { id: docSnapShot.id, ...product, category: c?.category, postedBy:"hello" } };
                } catch (error: any) {
        ...
        postProduct: builder.mutation<{ id: string | undefined; },Product>({
